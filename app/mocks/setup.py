...
def set_up_sample_bom_files(number):
    while number:
        number -= 1
        bom = BomFile(name=name.first_name(), comment=lorem_ipsum.paragraph(sentences_quantity=3))
        db.session.add(bom)
...
def set_up_sample_bom_file_data(id=1, qty=4):
    ...
    while qty:
        qty -= 1
        contents: BomFileContents = BomFileContents(item_no=basic.number(at_least=1, at_most=10),
